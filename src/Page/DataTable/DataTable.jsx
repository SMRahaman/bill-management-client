...
const DataTable = ({ filterReport, reportType }) => {
  ...
  const totalAmount = filterReport.reduce(
  ...
  return (
    <div>
      <div className="my-3 ">
        <button onClick={pdfHandler} className="btn btn-primary">
          Export PDF
        </button>
      </div>
      <div>
        <table class="table border text-[10px] my-5" id="data-table">
          <thead className="text-[10px] text-center">
            <tr>
              ...
              <th className="border border-slate-400">Purchase Date</th>
          ...
          <tbody className="text-center table-xs ">
            ...
            <td className="border border-slate-400" colSpan={12}>
              <div className="text-right mt-0 font-bold">
