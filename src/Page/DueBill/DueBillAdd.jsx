...
const DueBillAdd = () => {
  ...
  const subTotal=items.reduce((acc,currentVlaue)=>{
   ...
   if(donate==='Yes'&& purchaseType !=='Advanced'){
      ...
      totalAmount=parseFloat(subTotal-(parseFloat(advancePayment)+parseFloat(donateAmount))).toFixed(2)
    }
  ...
  const handelBillAdd = (e) => {
    ...
    if(billAndVoucher){
      axios.post('https://bill-deposite-server.vercel.app/api/due-bill',billAndVoucher)
      .then(res=>{
  ...
  return (
    <div className="pt-20">
      <form onSubmit={handelBillAdd}>
        ...
        <div className=" grid grid-cols-2 gap-5 mt-5">
         {purchaseType === "Creadit" && (
            ...
            <input
              ...
              className={purchaseType==='Creadit'? "input input-bordered w-full block max-w-xs":'hidden'}
            />
            </div>
          )}
          ...
          <div key={index} className="mt-5">
            ...
            <input
              ...
              <input
                ...
                className="input input-bordered w-full max-w-xs"
                value={parseFloat(product.itemQuantity*product.unitPrice).toFixed(2)}
              />
              </div>
              <div className="flex flex-col gap-2">
        ...
        <div>
          <div className="mt-5">
            <p className="text-sm text-right font-bold">Sub-Total:{parseFloat(subTotal).toFixed(2)} Tk</p>
            <div>
              {
                purchaseType==='Advanced'&&(
                  <p className="text-sm text-right font-bold">Advanced Payment:{advancePayment==='NaN'?0.00:advancePayment} Tk</p>
                )
              }
            </div>
            <div>
              {
                donate==='Yes'&&(
                  <p className="text-sm text-right font-bold">Donate Amonut:{donateAmount} Tk</p>
                )
              }
            </div>
           <div>
                <p className="text-sm text-right font-bold">Due Amount:{totalAmount?totalAmount:subTotal}Tk</p>
           </div>
          </div>
        </div>
