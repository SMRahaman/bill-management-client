...
const Dashboard = () => {
  ...
  const superAdminFilter = userData?.filter(
    (user) => user.role === "SuperAdmin"
  );
  const logisticFilter = userData?.filter(
    (user) => user?.role === "Logistic-User"
  );
  const accountsFilter = userData?.filter(
    (user) => user?.role === "Account-User"
  );
  const userWiseSupplier = supplierData?.filter(
    (sup) => sup?.userEmail === user?.email
  );
  const totalBillSubmitUserWise = dueBillData?.filter(
    (bill) => bill?.userName === user?.displayName
  );
        ...
        <div className="mt-20 grid grid-cols-3 gap-3">
          ...
          <div className="bg-gray-700 w-[300px] p-5 rounded text-center text-white">
            ...
            <p className="text-2xl font-bold">
              {totalBillSubmitUserWise.length}
            </p>
          </div>
        </div>
      )}
